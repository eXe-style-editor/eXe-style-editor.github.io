...
## EdEX Maintenance Note: Quick Settings and Click Editing

In this editor there are two different override layers inside `style.css`:

- `quick-overrides`: generated from the safe controls in the **Ajustes** tab
- `click-overrides`: generated from the click-to-edit tool in the preview

### Rule for future changes

When a click edit is clearly equivalent to a global quick setting, the editor should also synchronize:

- `state.quick`
- the visible control in the quick settings UI

This synchronization must **not** depend on an exact selector string match.  
It should accept equivalent selectors that are more specific, for example selectors scoped to a concrete iDevice type, as long as they still represent the same global visual block.

### Safe selector-to-quick mappings

These mappings are considered safe and global:

| Functional block | Typical selectors | Quick keys |
|------------------|-------------------|------------|
| iDevice content | `.box-content`, `.iDevice_content`, `.iDevice_inner` and specific typed variants ending in those selectors | `boxBgColor`, `boxTextAlign`, `boxFontSize` |
| iDevice title | `.box-title`, `.iDeviceTitle` and specific typed variants ending in those selectors | `boxTitleColor`, `boxTitleSize` |
| Content buttons | `button:not(.toggler):not(.box-toggle)` and scoped variants ending in that selector | `buttonBgColor`, `buttonTextColor` |
| Side menu container | `#siteNav` | `menuBgColor` |
| Side menu links | `#siteNav a` | `menuTextColor` |
| Active menu link | `#siteNav a.active` | `menuActiveBgColor`, `menuActiveTextColor` |
| Page title | `.page-title` | `titleColor`, `pageTitleSize`, `pageTitleWeight` |
| Project title | `.package-title` | `packageTitleColor`, `packageTitleSize`, `packageTitleWeight` |

### Cases that should not be auto-synced

Do not sync automatically when the click edit is ambiguous or more local than the quick settings model:

- edits scoped only to one particular component when there is no clean global equivalent
- mixed interactive edits applied as one block (`:hover`, `:focus`, `:active`)
- selectors whose visual effect depends on structure rather than a single reusable block

### Current known limitation

`Fondo iDevice` is still a delicate case.  
The CSS generated by click editing is correct, but the left quick color control may fail to reflect the new value when the edited selector is more specific than the global iDevice content selector.

If this area is revisited, prefer improving the selector-equivalence matching rather than forcing more aggressive global synchronization.
